...
let expName = '2AFC_3';  // from the Builder filename that created this script
let expInfo = {
    'Prolific_ID': '',
    'Age': '',
    'Gender': '',
...
const pic_rememberingLoopScheduler = new Scheduler(psychoJS);
flowScheduler.add(pic_rememberingLoopBegin(pic_rememberingLoopScheduler));
flowScheduler.add(pic_rememberingLoopScheduler);
flowScheduler.add(pic_rememberingLoopEnd);
...
psychoJS.start({
  ...
  resources: [
    // resources:
    {'name': '2AFC_3.xlsx', 'path': '2AFC_3.xlsx'},
...
async function updateInfo() {
  ...
  psychoJS.setRedirectUrls('https://app.prolific.com/submissions/complete?cc=C1JKK434', '');
...
var pic_remembering;
function pic_rememberingLoopBegin(pic_rememberingLoopScheduler, snapshot) {
  return async function() {
    TrialHandler.fromSnapshot(snapshot); // update internal variables (.thisN etc) of the loop
    
    // set up handler to look after randomisation of conditions etc
    pic_remembering = new TrialHandler({
      psychoJS: psychoJS,
      nReps: 1, method: TrialHandler.Method.RANDOM,
      extraInfo: expInfo, originPath: undefined,
      trialList: '2AFC_3.xlsx',
      seed: undefined, name: 'pic_remembering'
    });
    psychoJS.experiment.addLoop(pic_remembering); // add the loop to the experiment
    currentLoop = pic_remembering;  // we're now the current loop
    
    // Schedule all the trials in the trialList:
    for (const thisPic_remembering of pic_remembering) {
      snapshot = pic_remembering.getSnapshot();
      pic_rememberingLoopScheduler.add(importConditions(snapshot));
      pic_rememberingLoopScheduler.add(remeberingRoutineBegin(snapshot));
      pic_rememberingLoopScheduler.add(remeberingRoutineEachFrame());
      pic_rememberingLoopScheduler.add(remeberingRoutineEnd(snapshot));
      pic_rememberingLoopScheduler.add(pic_rememberingLoopEndIteration(pic_rememberingLoopScheduler, snapshot));
    }
...
async function pic_rememberingLoopEnd() {
  // terminate loop
  psychoJS.experiment.removeLoop(pic_remembering);
...
function pic_rememberingLoopEndIteration(scheduler, snapshot) {
...
function remeberingRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of remeberingComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function BeforeExpRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of BeforeExpComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
